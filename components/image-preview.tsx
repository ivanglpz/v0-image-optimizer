...
export function ImagePreview({ image, onRemove }: ImagePreviewProps) {
  ...
  return (
    <Card className="overflow-hidden bg-card flex-shrink-0 py-0 w-72 h-auto border-0 leading-7 gap-0">
      <div className="relative h-48 bg-muted">
        <img src={image.optimized || image.preview} alt={image.file.name} className="w-full h-full object-contain" />
        <Button
          variant="destructive"
          size="icon"
      ...
      <div className="p-4 space-y-2 py-4">
        <div className="space-y-1">
          ...
          <div className="flex items-center gap-2 text-xs text-muted-foreground font-mono">
            ...
            {image.optimizedSize && (
              <>
                <span>→</span>
                <span className="text-accent font-semibold">{formatSize(image.optimizedSize)}</span>
        ...
        {image.optimized && (
          <Button variant="outline" size="sm" className="w-full gap-2 bg-transparent" onClick={handleDownload}>
